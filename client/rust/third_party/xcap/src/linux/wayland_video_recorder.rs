...
/// RE:LINK PATCH: SelectSources responds with an empty vardict.
#[derive(DeserializeDict, Type, Debug)]
#[zvariant(signature = "dict")]
pub struct ScreenCastSelectSourcesResponse {}
...
impl ScreenCast<'_> {
    ...
    pub fn select_sources(&self, session: &OwnedObjectPath) -> XCapResult<()> {
        ...
        // RE:LINK PATCH: subscribe to Response before the method call and
        // actually wait for it. The portal answers SelectSources within
        // milliseconds (no user interaction), so subscribing afterwards
        // misses the signal; and Start must not race an unfinished
        // SelectSources.
        let receiver = wait_zbus_response::<ScreenCastSelectSourcesResponse>(&portal_request);
