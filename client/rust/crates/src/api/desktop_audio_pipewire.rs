//! PipeWire implementation behind `desktop_audio`'s platform-independent
//! API. Loops back whatever
...
pub(crate) async fn start_capture(connection_id: String) -> anyhow::Result<()> {
    {
...
pub(crate) fn is_active(connection_id: &str) -> bool {
    CAPTURES.lock().unwrap().contains_key(connection_id)
}

/// Tear down capture when the WebRTC session closes.
pub(crate) fn stop_for_connection(connection_id: &str) {
